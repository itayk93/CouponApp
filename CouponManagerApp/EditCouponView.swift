...
struct EditCouponView: View {
    ...
    init(coupon: Coupon, onUpdate: @escaping () -> Void) {
        ...
        _value = State(initialValue: PricingFormatter.string(from: coupon.value))
        _cost = State(initialValue: PricingFormatter.string(from: coupon.cost))
        ...
        _discountPercentage = State(initialValue: PricingFormatter.string(from: discountValue))
